# جدول المحتويات

* [البدء](README.md)
* [GUI : التنقل](navigation.md)
* [GUI : المشاريع](projects.md)
* [CLI: سطر الأوامر](CLI.md)
* [API: Python SDK](api-python-sdk.md)
* [الكاميرات المدعومة](supported-cameras.md)
* [تنسيقات الصور الناتجة](output-image-formats.md)
* [Chloros+ تسجيل الدخول](chloros+-login.md)
* [أهداف المعايرة](calibration-targets.md)
* [اللغات المدعومة](supported-languages.md)
* [تنزيل](download.md)
* [الأسئلة الشائعة](faq.md)

## معالجة الصور (GUI)

* [ضبط إعدادات المشروع](processing-images-gui/adjusting-project-settings.md)
* [إضافة ملفات إلى مشروع](processing-images-gui/adding-files-to-a-project.md)
* [اختيار الصور المستهدفة](processing-images-gui/choosing-target-images.md)
* [بدء المعالجة](processing-images-gui/starting-the-processing.md)
* [مراقبة المعالجة](processing-images-gui/monitoring-the-processing.md)
* [إنهاء المعالجة](processing-images-gui/finishing-the-processing.md)

## إعدادات المشروع

* [إعدادات المشروع](project-settings/project-settings.md)
* [صيغ المؤشر متعدد الأطياف](project-settings/multispectral-index-formulas.md)

## عارض الصور (GUI)

* [شبكة الصور](image-viewer-gui/image-grid.md)
* [فتح صورة بملء الشاشة](image-viewer-gui/opening-an-image-full-screen.md)
* [طبقات الصورة](image-viewer-gui/image-layers.md)
* [مؤشر/LUT Sandbox](image-viewer-gui/index-lut-sandbox.md)
* [علامات الخريطة](image-viewer-gui/map-markers.md)
