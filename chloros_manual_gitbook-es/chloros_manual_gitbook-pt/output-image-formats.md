---
metaLinks:
  alternates:
    - https://app.gitbook.com/s/o044KN3Ws0uIDvOmSkcR/output-image-formats
---

# تنسيقات صور الإخراج

### تيف (16 بت)

إخراج تنسيق TIFF (.tif) 16 بت (رقم رقمي للبكسل 0 - 65535).

### TIFF (32 بت، النسبة المئوية %)

إخراج تنسيق TIFF (.tif) 32 بت (تعويم البكسل 0.0 - 1.0). تمثل قيم البكسل نسب الانعكاس حيث 0.0 = 0% و1.0 = 100%.

### **JPG (8 بت)**

إخراج تنسيق 8 بت (رقم البكسل الرقمي 0 -255) JPG (.jpg).

### ** PNG (8 بت) **

إخراج بتنسيق PNG (.png) بحجم 8 بت (رقم رقمي للبكسل 0 - 255). ضغط بدون فقدان مناسب لعرض الويب والتصور.

