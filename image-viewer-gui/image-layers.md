...
* تأكيد اكتشاف أهداف المعايرة بشكل صحيح
* التحقق من جودة صورة الهدف
* استكشاف أخطاء المعايرة وإصلاحها

{% hint style=&quot;info&quot; %}
**طبقة الهدف**: تظهر هذه الطبقة فقط في القائمة المنسدلة للصور التي تحتوي على أهداف معايرة. لن تتوفر هذه الخيار في الصور العادية.
{% endhint %}
...
* صورة مؤشر الغطاء النباتي المحسوب (NDVI في هذا المثال)
...
عند التنقل إلى صورة مختلفة (باستخدام مفاتيح الأسهم أو النقر فوق الصور المصغرة):
...
4. قم بالتبديل إلى **RAW (انعكاس)** من القائمة المنسدلة
...
* **النطاق**: 0-255 (8 بت)
* **المعنى**: عرض القيم، مصححة بواسطة جاما
...
* **المعنى**: النسبة المئوية للانعكاس المعايرة
* **الاستخدام**: القياسات والتحليلات العلمية

**بالنسبة إلى 16 بت TIFF:** اقسم على 65,535 للحصول على النسبة المئوية للانعكاس **بالنسبة إلى 32 بت النسبة المئوية:** تمثل القيم النسبة المئوية مباشرة (0.5 = 50٪ انعكاس)
...
* **تحقق دائمًا من RAW (الأصلية)**: تحقق من جودة البيانات المصدرية قبل الوثوق بالنتائج المعالجة
* **قارن الطبقات**: استخدم تبديل الطبقات للتحقق من صحة المعالجة
* **تحقق من نطاقات الفهرس**: استخدم وضع Pixel Percent مع طبقات الفهرس للتحقق من معقولية القيم
...
2. تحقق من إعدادات المشروع للتأكد من تكوين الفهارس
3. أعد المعالجة مع تمكين الفهارس المطلوبة
...
4. راجع [اختيار صور الهدف](../processing-images-gui/choosing-target-images.md)
...
انظر [صندوق الحماية للفهرس/LUT](index-lut-sandbox.md) للحصول على التفاصيل.
