...
تتيح لك القائمة المنسدلة &quot;طبقات الصورة&quot; في عارض الصور Chloros التبديل بسرعة بين الإصدارات المختلفة لنفس الصورة - من اللقطات الأصلية إلى مخرجات الانعكاس المعالجة والصور الفهرسية المحسوبة.
...
* صورة JPG الأصلية المعاينة من الكاميرا
...
* معاينة سريعة للصورة الأصلية
* التحقق من تكوين الصورة وتأطيرها
* التحقق من جودة الصورة قبل المعالجة
...
* تظهر فقط للصور التي تم تحديدها على أنها تحتوي على أهداف معايرة
* تعرض الصورة RAW الأصلية مع الهدف المكتشف
* تُستخدم للتحقق من نجاح اكتشاف الهدف
...
* في حالة عرض &quot;RAW (الانعكاس)&quot;، تعرض الصورة التالية &quot;RAW (الانعكاس)&quot; (إذا كان متاحًا)
* في حالة عرض &quot;RAW (NDVI الفهرس)&quot;، تعرض الصورة التالية &quot;RAW (NDVI Index)&quot; (إذا كانت متوفرة)
* إذا لم تكن الطبقة نفسها موجودة، يتم التعيين الافتراضي إلى JPG
...
* **معرفة اختصارات لوحة المفاتيح**: على الرغم من عدم وجود اختصارات لوحة المفاتيح للطبقات، فإن أسهم التنقل (←/→) تعمل عبر جميع الطبقات
...
* **تحقق من نطاقات الفهرس**: استخدم وضع Pixel Percent مع طبقات الفهرس للتحقق من أن القيم معقولة
...
1. تحقق من معالجة الصورة (تحقق من مجلد المخرجات بحثًا عن الملفات المعالجة)
2. تحقق من إعدادات المشروع للتأكد من تكوين الفهارس
3. أعد المعالجة مع تمكين المؤشرات المطلوبة

### عرض طبقة خاطئة

**المشكلة**: الصورة تفتح في طبقة غير متوقعة

**السبب**: تفضيل الطبقة من الصورة السابقة تم نقله، ولكن هذه الطبقة غير موجودة في الصورة الحالية

**الحل**: Chloros يعود تلقائيًا إلى JPG عندما تكون الطبقة المفضلة غير متاحة - وهذا سلوك طبيعي
...
* **وضع نسبة البكسل**: التبديل بين عرض DN والنسبة المئوية
