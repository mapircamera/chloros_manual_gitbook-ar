...
يمكن الوصول إلى صندوق الحماية للمؤشر/LUT من علامة التبويب **شريط الجانب** في **عارض الصور** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> :
...
{% hint style=&quot;info&quot; %}
**التحجيم التلقائي**: عند تطبيق LUT لأول مرة، يقوم Chloros تلقائيًا بتعيين الحد الأدنى/الحد الأقصى إلى نطاق البيانات الفعلي في الصورة. يمكنك بعد ذلك تضييق هذا النطاق للتركيز على نطاقات قيم محددة تهمك.
{% endhint %}

**أمثلة على تعديلات نطاق NDVI:**
...
* البكسلات **خارج النطاق** تصبح **شفافة تمامًا**
* فقط البكسلات **داخل النطاق** تظهر تدرج الألوان
* **حالة الاستخدام**: تراكب GIS، عزل نطاقات قيم محددة، إبراز المناطق ذات الأهمية فقط
* **مثال**: عرض NDVI 0.4-0.7 بالألوان فقط، وجميع العناصر الأخرى شفافة
...
* يتم عرض البكسلات **خارج النطاق** **باللون الرمادي** (مع عرض قيم المؤشر الأولية)
* تظهر البكسلات **داخل النطاق** **تدرجًا لونيًا**
* **حالة الاستخدام**: تمييز خفيف، الحفاظ على السياق مع التأكيد على المناطق المهمة
* **مثال**: تمييز النباتات المتضررة بالألوان (NDVI 0.3-0.5) مع إظهار المناطق السليمة باللون الرمادي

#### 4. الخلفية الأصلية

* تعرض البكسلات **خارج النطاق** **الصورة متعددة الأطياف الأصلية**
* تعرض البكسلات **داخل النطاق** **تدرج لوني**
...
| **خلفية أصلية**    | التقارير، العروض التقديمية، التحليل البديهي | لون على النطاق، صورة خارج النطاق |
...
5. اختر لونًا جديدًا باستخدام:
   ...
   * **إدخال رمز سداسي عشري**: مواصفات اللون الدقيقة (على سبيل المثال، `#FF0000` للأحمر)
6. انقر خارج منتقي الألوان **لتطبيق اللون الجديد**
7. يتم **تحديث التدرج اللوني على الفور** على الصورة

**إضافة أو إزالة نقاط توقف الألوان:**

* **إضافة توقف**: انقر على أيقونة + لإضافة عينة جديدة في النهاية
* **إزالة توقف**: انقر نقرًا مزدوجًا على مربع اللون لإزالة العينة
...
* **مناسب لضعاف البصر**: استخدم تركيبات البرتقالي والأزرق أو الأرجواني والأصفر
* **تحسين الطباعة**: اختر ألوانًا تناسب الطباعة بالألوان والطباعة بالدرجات الرمادية
* **عتبة متعددة**: استخدم ألوانًا مميزة عند عتبات قيم محددة للتصنيف

{% hint style=&quot;info&quot; %}
**حفظ التدرجات المخصصة**: يمكن حفظ التدرجات المخصصة وإعادة استخدامها. انقر على أيقونة الحفظ في لوحة LUT للحفاظ على أنظمة الألوان المخصصة لاستخدامها في المستقبل.
{% endhint %}
...
* **تعديل القص** → تغيير عرض الخلفية على الفور
* **تحرير الألوان** → تطبيق التدرج المخصص على الفور
...
7. **صقل الألوان** - خصص التدرج إذا لزم الأمر للتأكيد على شيء معين
8. **إنهاء الإعدادات** - قم بتوثيق الإعدادات وانسخها إلى إعدادات المشروع لمعالجة التصدير
...
* **الهدف**: تصور صحة النبات فقط (باستثناء التربة)
